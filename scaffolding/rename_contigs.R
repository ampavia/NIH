setwd("/scratch/ac05869/gese_final_yahs/gese_v2.asm/no_filter/")
library(readr)
library(dplyr)
library(stringr)

gese_v2 <- read_tsv("./gese_v2.tsv", col_names = c("name", "length", "seq"))
renamed <- gese_v2  %>%
  mutate(order = c(1:226)) %>% #depends on total number of contigs in asm file
  ...
  mutate(name = case_when(
    ...
    T ~ "NA")) %>%
  mutate(name = case_when(name == "NA" ~ paste(">scf", order2),
                                                     T ~ name)) %>%
  select(name, seq)
