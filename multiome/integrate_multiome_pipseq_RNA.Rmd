...
head(anno)
```
#Write gene ID's as real gene ID's
```{r}
gene_identities <- repr_gene_models %>% 
  dplyr::filter(X3 == "mRNA") %>% 
  dplyr::select(X1, X4, X5, X7, X9) %>% 
  separate(X9, c("ID", "Name"), sep = ";") %>% 
  dplyr::select(-Name) %>% 
  separate(ID, c("temp", "gene"), sep = "=") %>% 
  dplyr::select(-temp) %>% 
  mutate(gene_ID = str_replace(gene, "_", "-")) %>% 
  mutate(gene_ID = str_replace(gene_ID, "\\.\\d+$", "")) %>%
  dplyr::rename(
    Chr = X1,
    start = X4,
...
head(gene_identities)
dim(gene_identities) #103160
```
...
cluster_avg.data <- readRDS(file = "/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Data/KRT_RNA_cluster_expression.rds")

cluster_avg.data_table <- cluster_avg.data %>%
  as.data.frame() 
dim(cluster_avg.data_table) #60140

cluster_avg.data_table <- cluster_avg.data_table %>%
  rownames_to_column(var = "gene_ID") %>%
  left_join(gene_identities, by = "gene_ID") %>%
  select(gene, !gene_ID)
dim(cluster_avg.data_table)#60140
head(cluster_avg.data_table)
  
write_csv(cluster_avg.data_table, file = "/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Results/KRT_RNA_integrated_cluster_av_expression_repr_transcript.csv") #For plantismash gene coexpression
...
write_delim(cluster_avg.data_anno, file = "../Integrated_RNA_Results/KRT_RNA_integrated_cluster_av_expression_annotated.txt", delim = "\t")
```
...
KRT_RNA_markers
dim(KRT_RNA_markers)
KRT_RNA_markers_table <- KRT_RNA_markers %>%
...
KRT_RNA_markers_table
dim(KRT_RNA_markers_table)

write_delim(KRT_RNA_markers_table, "../Integrated_RNA_Results/KRT_RNA_integrated_markers.txt", delim = "\t")
write_csv(KRT_RNA_markers_table, "../Integrated_RNA_Results/KRT_RNA_integrated_markers.csv")

KRT_RNA_markers_anno <- KRT_RNA_markers_table %>%
  left_join(anno, by = "gene_ID") %>%
  select(gene_ID, cluster, p_val, p_val_adj, avg_log2FC, pct.1, pct.2, functional_annotation, start,end,strand)
write_delim(KRT_RNA_markers_anno, "../Integrated_RNA_Results/KRT_RNA_integrated_markers_annotated.txt", delim = "\t")
write_csv(KRT_RNA_markers_anno, "../Integrated_RNA_Results/KRT_RNA_integrated_markers_annotated.csv")
```
...
#Write GEX table with properly formatted gene_ID's
```{r}
repr_gene_models <- read_delim("../../genome/mitr_v1.working_models.repr.gff3",
                               delim = "\t", col_names = F)
dim(repr_gene_models) #1152702

repr_work_model_genes <- repr_gene_models %>% 
  dplyr::filter(X3 == "mRNA") %>% 
  dplyr::select(X1, X4, X5, X7, X9) %>% 
  separate(X9, c("ID", "Name"), sep = ";") %>% 
  dplyr::select(-Name) %>% 
...
head(repr_work_model_genes)
dim(repr_work_model_genes) #103160
```
