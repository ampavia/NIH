...
2>${YAHS}/${PREF}_tmp_juicer_pre.log \
| LC_ALL=C sort -k2,2d -k6,6d -T ${YAHS} --parallel=8 -S32G \
| awk 'NF' > ${YAHS}/${PREF}_alignments_sorted.txt.part) \
&& (mv ${YAHS}/${PREF}_alignments_sorted.txt.part ${YAHS}/${PREF}_alignments_sorted.txt)
...
#####
cat ${YAHS}/${PREF}_tmp_juicer_pre.log | grep "PRE_C_SIZE" | cut -d' ' -f2- >${YAHS}/${PREF}_scaffolds_final.chrom.sizes

>&2 echo "### Step 3: do juicer hic map"
(java -jar -Xmx32G $EBROOTJUICEBOX/juicer_tools.1.9.9.jar pre ${YAHS}/${PREF}_alignments_sorted.txt ${YAHS}/${PREF}.hic.part ${YAHS}/${PREF}_scaffolds_final.chrom.sizes) \
...
juicer pre -a -o ${JBAT}/${PREF}_JBAT ${YAHS}/${PREF}.bin ${YAHS}/${PREF}_scaffolds_final.agp ${REF}.fai 2>${JBAT}/${PREF}_tmp_juicer_pre_JBAT.log
