...
table(Idents(AA))
# Cells in each replicate: 6553
table(AA$orig.ident)
#How does cluster membership vary by replicate? 17 total clusters
table(Idents(AA), AA$orig.ident)

#Cells in each cluster
table(Idents(AB))
# Cells in each replicate: 5525
table(AB$orig.ident)
#How does cluster membership vary by replicate? 16 clusters
...
DimPlot(AA_doublet_test,pt.size = 1,label=TRUE, label.size = 5,reduction = "umap",group.by = "DF.classifications_0.25_0.01_459")+theme(aspect.ratio = 1)
sum(str_count(AA_doublet_test$DF.classifications_0.25_0.01_459, "Doublet"))

# 459 doublets out of 6553 cells (KRT_AA)
...
## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
#Update pK to be pK choosen from above steps!
AB_doublet_test <- doubletFinder(AB, PCs = 1:50, pN = 0.25, pK = 0.06, nExp = nExp_poi, reuse.pANN = FALSE, sct = FALSE)

AB_doublet_test # 387 doublets out of 5525 cells
...
DimPlot(AB_doublet_test,pt.size = 1,label=TRUE, label.size = 5,reduction = "umap",group.by = "DF.classifications_0.25_0.06_387" )+theme(aspect.ratio = 1)
sum(str_count(AB_doublet_test$DF.classifications_0.25_0.06_387, "Doublet"))
...
```{r}
#AA
AA_clean <- subset(AA_doublet_test, subset = DF.classifications_0.25_0.01_459 == "Singlet") #AA DF.classifications_0.25_0.005_1013
AA_clean # 6553 --> 6094 cells
#AB
AB_clean <- subset(AB_doublet_test, subset = DF.classifications_0.25_0.06_387 == "Singlet") #AB DF.classifications_0.25_0.01_802
