...
cluster.averages.MPI <- AverageExpression(leaf.combined.split.MPI, return.seurat = TRUE)
cluster.averages.MPI

# How can I extract expression matrix for all Cluster 0 cells (perhaps, to load into another package)
cluster_avg_MPI.data <- as.matrix(GetAssayData(cluster.averages.MPI, layer = "data")[, ])
write.table(cluster_avg_MPI.data, file = "/scratch/ac05869/MIT_KRT_sc_analysis/seurat_out/KRT_MIT_clean_integrated_MPI_cluster_avg_normalized_counts_28mar25.txt", sep = "\t", quote = FALSE)
...
cluster.averages.UGA <- AverageExpression(leaf.combined.split.UGA, return.seurat = TRUE)
