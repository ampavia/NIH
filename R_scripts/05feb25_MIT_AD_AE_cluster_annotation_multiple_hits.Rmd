...
gene_anno <- read_delim("/scratch/ac05869/nih/kratom/mitr_v1_anno/mitr_v1.functional_annotation.txt",
                               delim = "\t", col_names = c("gene_ID", "annotation"))
head(gene_anno)
```
...
kratom_genes <- read_delim("/scratch/ac05869/nih/marker_genes_dionne/blast/bestHits_blastn_hits.34761323.tsv", col_names = c("query", "gene_ID", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore"))
head(kratom_genes)
dim(kratom_genes) #32; bestHits is the top hit of multiple blast hits
kratom_genes$query <- gsub("\\.\\d+$", "", kratom_genes$query) #remove transcript syntax from arabidopsis genes

stem_markers <- markers %>% 
  left_join(kratom_genes, by = "query") %>% #merge dionne's marker genes and kratom blast hits by arabidopsis genes
  rbind(list("IPAP", "7DLH", "CRO_03G018360", "Li et al.", "7-deoxyloganic acid hydroxylase", "Mitsp.v1.21_1G007810.1", 75.085, 293, 65, 8, 1530, 1818, 991, 1279, 3.50e-28, 130)) %>% #manually added the top hit for Li's IPAP marker from Cro_v3 blast run
  group_by(gene_ID) %>%
  ...
  select(c(1, 3, 6, 2, 5, 4, 7, 17:20)) %>%
  left_join(gene_anno, by = "gene_ID", multiple = "first") %>%
  mutate(gene_ID = str_replace(gene_ID, "_", "-")) %>%
  mutate(gene_ID = str_replace(gene_ID, "\\.\\d+$", ""))

#write_csv(leaf_markers, "/scratch/ac05869/nih/marker_genes_dionne/Annotated_markers_TAIR_Cro_v1_Mitr_v1.csv") #This is a table of the list of kratom genes that had a match in arabidopsis, cro_v3, and its annotation
...
# View Seurat cluster genes and merge with annotation file
```{r}
stem.combined.markers2 <- stem.combined.markers %>%
  rename(gene_ID = gene) #I want to merge with annotation data in order to know what cluster's are what cell type
head(stem.combined.markers2)

gene_anno$gene_ID <- gsub("\\.\\d+$", "", gene_anno$gene_ID)
gene_anno$gene_ID <- gsub(pattern = "_", replacement = "-", gene_anno$gene_ID)

cluster_anno <- left_join(stem.combined.markers2, gene_anno, by = "gene_ID", multiple = "first")
...
stem_markers2 <- stem_markers %>% 
  dplyr::select(symbol, Full_name, cell_type, gene_ID, annotation) %>% 
  # rbind(stem_markers) %>% 
...
#  mutate(gene_ID = str_sub(gene_ID, end = 15)) %>% 
  full_join(stem_markers2 %>% 
   ...
   mutate(cell_type2 = case_when(
    ...
    str_detect(cell_type, "xyl") ~ "X",
    str_detect(cell_type, "phl") ~ "Ph",
    str_detect(cell_type, "com") ~ "Com",
    str_detect(cell_type, "vas | tra") ~ "V",
...
#View this and compare with cluster_anno df
```
