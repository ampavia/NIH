...
Both found in top 5000 variable genes.
...
bait_var <- all_var_and_ranks %>% 
  filter(gene_ID %in% Baits$X2) %>% 
  group_by(gene_ID) %>% 
  slice_max(n = 1, order_by = var)

bait_var #var = 0.14 and 0.096. rank ~61,600
...
ggsave("/scratch/ac05869/nih/coexpression/kratom_stem/Results/gene_var_distribution.svg", height = 3.5, width = 3.5)
ggsave("/scratch/ac05869/nih/coexpression/kratom_stem/Results/gene_var_distribution.png", height = 3.5, width = 3.5)
```
From this graph, you can see a couple things:

Our bait genes are among the most highly variable genes, and thus highly ranked.
If we just take the top 5000 genes, it takes pretty much the entire upper elbow of the graph.
...
saveRDS(edge_table, file = "/scratch/ac05869/nih/coexpression/kratom_stem/Results/MIT_AD_AE_edge_table.rds")
```
...
If you cut off the FDR at 0.05, then your r values are 0.511 or larger. 
If you cut off the FDR at 0.01, then your r values are 0.627  or larger. 
...
These two bait genes (SLTr and TDC) are chosen based on that they are involved in the same process (Iridoid synthesis)
They have a r value of 0.89, which is rather high, considering at FDR < 0.01, r cutoff was 0.627. 

Base on this empirical observation, we can say we cut off at the vicinity of 0.89, maybe r > 0.8. 
Note that this is way more stringent than cutting off at FDR < 0.01 (r > 0.627). 
...
ggsave("/scratch/ac05869/nih/coexpression/kratom_stem/Results/r_histogram_r0.8.svg", height = 3.5, width = 5, bg = "white")
ggsave("/scratch/ac05869/nih/coexpression/kratom_stem/Results/r_histogram_r0.8.png", height = 3.5, width = 5, bg = "white")
```
...
We are now down to 160198 edges.
...
We have 4810 genes in this network, along with 160198 edges.
...
Looks like there are ~27 modules that have 5 or more genes, comprising ~4523 genes. 
Not all genes are contained in modules. They are just lowly connected genes. 
4523/4810 = 94% of the genes in the network are assigned to clusters with 5 or more genes. 
...
```{r}
Exp_table_long_averaged_z_high_var_modules <- Exp_table_long_averaged_z %>% 
...
write_excel_csv(Exp_table_long_averaged_z_high_var_modules, 
                "/scratch/ac05869/nih/coexpression/kratom_stem/Results/Exp_table_long_averaged_z_modules_r0.8.csv")
```
...
* module 21, 15, 35: Clusters 9, 11, 20. Where the dotplot showed cell specificity for MIA genes
* module 15, where our bait genes are. 

```{r}
module_line_plot <- Exp_table_long_averaged_z_high_var_modules %>% 
  filter(module == "21" |
           module == "15" ) %>% 
  ggplot(aes(x = cluster, y = z.score)) +
