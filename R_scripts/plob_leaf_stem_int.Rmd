---
title: "Integrate plumeria leaf and stem single cell"
date: 4-June-2025
author: "Anne Pavia"
output:
  html_document:
...
cluster_avg.data_anno <- read_delim(file = "/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Results/KRT_RNA_integrated_cluster_av_expression.txt", delim = "\t")
cluster_avg.long <- read_delim(file = "/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Results/KRT_RNA_integrated_cluster_av_expression_long.txt", delim = "\t")
KRT_RNA_markers_anno <- read_delim("/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Results/KRT_RNA_integrated_markers.txt", delim = "\t")


#repr working model annotation
...
#write_delim(pathway_genes, file = "../Data/annotated_kratom_pathway.tsv", delim = "\t")
...
pathway_genes <- pathway_genes %>% mutate(tag2 = paste(tag, "-", gene_ID))

rev_pathway_genes <- pathway_genes[order(nrow(pathway_genes):1),]
head(rev_pathway_genes)

#Baits <- read_delim("../Data/allwin_goi_top_blast.txt", delim = "\t")
#Baits <- Baits %>% left_join(repr_anno, by = "gene_ID")
#head(Baits)
#write_delim(Baits, file = "../Data/annotated_allwin_goi_top_blast.txt")
...
anno <- read_delim("/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Data/gene_annotations.tsv", delim = "\t")
anno_markers <- read_delim("/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Results/R_output/annotated_ath_marker_genes_in_kratom.csv")
kratom_markers <- read_delim("/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Data/kratom_specific_markers")
```
...
rev_pathway_genes <- rev_pathway_genes %>% filter(gene_ID != "Mitsp.v1.03-2G022350") #TDC gene is not present in dataset
...
dotplot_data %>% 
  ...
  ggplot(aes(x = id, y = reorder(tag2, order, decreasing = TRUE))) +
...
ggsave("/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Results/KRT_RNA_pathway_genes_best_orthologs.svg", height = 10, width = 10, bg = "white")
ggsave("/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Results/KRT_RNA_pathway_genes_best_orthologs.png", height = 10, width = 10, bg = "white")
```
...
RNA_umap
ggsave("/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Results/KRT_leaf_cluster_annotation_UMAP.png", height = 7, width = 5, bg = "white")
ggsave("/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Results/KRT_leaf_cluster_annotation_UMAP.svg", height = 7, width = 5, bg = "white")
```
...
ggsave("/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Results/Cell_proportions.svg", height = 3, width = 6.5, bg = "white")
ggsave("/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Results/Cell_proportions.png", height = 3, width = 6.5, bg = "white")
```

```{r}
saveRDS(KRT_RNA, "/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Data/KRT_RNA_with_metadata.Rds")
KRT_RNA <- readRDS("/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Data/KRT_RNA_with_metadata.Rds")
```
...
```{r}
saveRDS(KRT_multiome, file = "/scratch/ac05869/10X_Multiome/KRT_leaf/seurat_out/Integrated_RNA_Data/KRT_RNA_multiome_with_metadata_24july25.rds")
```
