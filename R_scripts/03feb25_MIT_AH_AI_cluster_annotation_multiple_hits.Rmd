...
Marker_dot
ggsave("/scratch/ac05869/nih/pop_markers/kratom_leaf/MIT_AH_AI_Cambium_markers.png", width = 10, height = 8)
```
...
```{r}
list(cells)
cell <- "G0/G1" #filter by cell_type2 to see what markers are cluster specific
Marker_dot <- Marker_dotplot_nice_df %>% 
  filter(str_detect(cell_type2, cell) == T) %>% #filter to see what markers are cluster specific
  ggplot(aes(x = id, y = reorder(tag, cell_type3, decreasing = TRUE))) +
  geom_point(aes(size = pct.exp, fill = avg.exp.scaled), shape = 21, color = "white") +
...
Marker_dot
ggsave("/scratch/ac05869/nih/pop_markers/kratom_leaf/MIT_AH_AI_G0_G1_phase_markers.png", width = 10, height = 8)
```
```{r}
list(cells)
cell <- "G2" #filter by cell_type2 to see what markers are cluster specific
Marker_dot <- Marker_dotplot_nice_df %>% 
  filter(str_detect(cell_type2, cell) == T) %>% #filter to see what markers are cluster specific
  ggplot(aes(x = id, y = reorder(tag, cell_type3, decreasing = TRUE))) +
  geom_point(aes(size = pct.exp, fill = avg.exp.scaled), shape = 21, color = "white") +
...
Marker_dot
ggsave("/scratch/ac05869/nih/pop_markers/kratom_leaf/MIT_AH_AI_G2_phase_markers.png", width = 10, height = 8)
```
...
Marker_dot
ggsave("/scratch/ac05869/nih/pop_markers/kratom_leaf/MIT_AH_AI_SAM_markers.png", width = 10, height = 8)
```
...
* M: "0", "2", "3", "7", "8", 
* Epi: "1", "6", "10", "14", "16",
* Xylem: "12", "13", "18, (and "5", "19",)
* Vasc: "5", 19", (xylem)
* Phloem: "9, "15", (possibly 18 SUC2)
* Cambium: "12", 13",
* Companion cells: "9", "21", (phloem)
* IPAP: "20",
...
```{r}
leaf_markers2 <- leaf_markers %>% 
  dplyr::select(Primary_Gene_Symbol, cell_type, gene_ID) %>%
  dplyr::filter(str_detect(Primary_Gene_Symbol, remove) == F) %>%
...
Leaf_cell_type_mkr_text <- Marker_dotplot_nice_df %>% 
  ...
  geom_text(aes(label = unique(Primary_Gene_Symbol), fontface = "italic")) +
  theme_void()
