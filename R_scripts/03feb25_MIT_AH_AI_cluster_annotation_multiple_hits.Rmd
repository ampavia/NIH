...
```{r}
leaf_markers_D <- read_delim("/scratch/ac05869/nih/marker_genes_dionne/From_Dionne/dionne_marker_genes_list.tsv", col_types = cols())
head(leaf_markers_D)
dim(leaf_markers_D) #176

kratom_genes <- read_delim("/scratch/ac05869/nih/marker_genes_dionne/blast/blastn_hits.34716718.tsv", col_names = c("query", "gene_ID", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore"))
head(kratom_genes) 
dim(kratom_genes) #141
kratom_genes$query <- gsub("\\.\\d+$", "", kratom_genes$query)#Remove transcript syntax because Dionne's markers don't have them

leaf_markers <- leaf_markers_D %>% 
  left_join(kratom_genes, by = "query", multiple = "first") %>%
...
```{r}
leaf.combined.markers2 <- leaf.combined.markers %>%
  rename(gene_ID = kratom.id) #I want to merge with annotation data in order to know what cluster's are what cell type
head(leaf.combined.markers2)

gene_anno$gene_ID <- gsub("\\.\\d+$", "", gene_anno$gene_ID)
gene_anno$gene_ID <- gsub(pattern = "_", replacement = "-", gene_anno$gene_ID)

cluster_anno <- left_join(leaf.combined.markers2, gene_anno, by = "gene_ID", multiple = "first")
...
#  mutate(gene_ID = str_sub(gene_ID, end = 15)) %>% 
  ...
  mutate(cell_type2 = factor(cell_type2, levels = c(
    "M", "E", "GC", "V", "X", "Ph", "Cam", "Com", "IP", "I", "Pro", "G0/G1", "G2", "SAM", "TRI"
  ))) %>% 
...
leaf_markers2 <- leaf_markers %>% 
  ...
  dplyr::filter(str_detect(gene_ID, "Mitsp.v1.01-1G005390") == F) %>% #underexpessed DCR epidermis marker
