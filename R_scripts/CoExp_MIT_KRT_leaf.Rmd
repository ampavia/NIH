...
subnetwork_edges2 <- edge_table_select %>% 
  filter(from %in% names(neighbors_of_bait_0_17) &
           to %in% names(neighbors_of_bait_0_17)) %>% 
  group_by(from) %>% 
  slice_max(order_by = r, n = 5) %>% 
  ungroup() %>% 
  group_by(to) %>% 
...
length(subnetwork_edges3)
dim(subnetwork_edges3)
```


> 3604 nodes & 9375 edges
> 6 nodes & 7491 edges
> 6 nodes & 1960 edges
...
```{r}
 my_subnetwork1 %>% 
  ...
  labs(fill = "Bait genes", title = "All GOI nearest 5 neighbors") +
  guides(size = "none",
...
ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/subnetwork_cluster0&17_graph_24mar25.svg", height = 4.5, width = 10, bg = "white")
ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/subnetwork_cluster0&17_graph_24mar25.png", height = 4.5, width = 10, bg = "white")
```
...
```{r}
neighbors_of_CO1_10H_DCR_SAS <- c(
  neighbors(my_network, v = "Mitsp.v1.18-2G016780"), #MsSAS 
  neighbors(my_network, v = "Mitsp.v1.10-1G010920"), #MsDCR1
  neighbors(my_network, v = "Mitsp.v1.10-2G012340"), #MsDCR2
  neighbors(my_network, v = "Mitsp.v1.22-2G007110"), #Ms10H
...
length(neighbors_of_CO1_10H_DCR_SAS)
```
...
length(subnetwork_module_10_nodes$gene_ID) #3540
