...
```{r}
Baits <- read_delim("/scratch/ac05869/MIT_KRT_sc_analysis/leaf_baits_26feb25.txt", delim = "\t", col_names = F, col_types = cols())
...
head(Baits)
edge_table_select %>% 
  filter(str_detect(from, "Mitsp.v1.16-1G012780") & #MsLAMT top hit Iridoid pathway
           str_detect(to,"Mitsp.v1.04-2G008810") | #MsSLS top hit
         str_detect(from, "Mitsp.v1.04-2G008810") &
           str_detect(to,"Mitsp.v1.16-1G012780")  ) 

edge_table_select %>% 
  filter(str_detect(from, "Mitsp.v1.10-2G022130") & #MsCO1 MIA pathway
           str_detect(to,"Mitsp.v1.10-1G021390") | #MsCO2 MIA pathway
         str_detect(from, "Mitsp.v1.10-1G021390") &
           str_detect(to,"Mitsp.v1.10-2G022130")  )

edge_table_select %>% 
  filter(str_detect(from, "Mitsp.v1.10-1G021390") & #MsCO2 MIA pathway
           str_detect(to,"Mitsp.v1.08-2G007110") | #Ms10H
         str_detect(from, "Mitsp.v1.08-2G007110") &
...
edge_table_select %>% 
  filter(str_detect(from, "Mitsp.v1.08-2G007110") & #Ms10H MIA pathway
           str_detect(to,"Mitsp.v1.10-2G012340") | #Red03
         str_detect(from, "Mitsp.v1.10-2G012340") &
           str_detect(to,"Mitsp.v1.08-2G007110")  ) 

edge_table_select %>% 
  filter(str_detect(from, "Mitsp.v1.08-2G007110") & #Ms10H MIA pathway
           str_detect(to,"Mitsp.v1.10-1G010980") | #DCR
         str_detect(from, "Mitsp.v1.10-1G010980") &
           str_detect(to,"Mitsp.v1.08-2G007110")  ) 
```


> Iridoid genes MsLAMT and MsSLS have correlation of r= 0.9185664
> MsCO1 and MsCO2 MIA genes have correlation of r= 0.9799718
> MsCO2 and Ms10H do not have correlation at this cutoff
> MsCO1 and Ms10H do not have correlation at this cutoff
> Ms10H and Red03 do not have correlation at this cutoff
> Ms10H and MsDCR do not have correlation at this cutoff
...
##### Left off here to consult with Josh ###

### Check bait genes that are more than 1 subsequent step away
```{r}
head(Baits)
edge_table_select %>% 
  filter(str_detect(from, "Mitsp.v1.16-1G012780") & #MsLAMT Iridoid pathway
           str_detect(to,"Mitsp.v1.10-2G022130") | #MsCO1 MIA pathway
         str_detect(from, "Mitsp.v1.10-2G022130") &
           str_detect(to,"Mitsp.v1.16-1G012780")  ) 

edge_table_select %>% 
  filter(str_detect(from, "Mitsp.v1.16-1G012780") & #MsLAMT Iridoid pathway
           str_detect(to,"Mitsp.v1.08-2G007110") | #Ms10H MIA pathway
         str_detect(from, "Mitsp.v1.08-2G007110") &
           str_detect(to,"Mitsp.v1.16-1G012780")  ) 

edge_table_select %>% 
  filter(str_detect(from, "Mitsp.v1.16-1G012780") & #MsLAMT Iridoid pathway
           str_detect(to,"Mitsp.v1.10-2G012340") | #Red03 MIA pathway
         str_detect(from, "Mitsp.v1.10-2G012340") &
           str_detect(to,"Mitsp.v1.16-1G012780")  ) 
...
edge_table_select2 %>% 
  filter(str_detect(from, "Mitsp.v1.10-2G016180") & #MsDCS1 alkaloid pathway
           str_detect(to,"Mitsp.v1.18-2G016770") | #SAS
         str_detect(from, "Mitsp.v1.18-2G016770") &
           str_detect(to,"Mitsp.v1.10-2G016180")  ) 

edge_table_select %>% 
  filter(str_detect(from, "Mitsp.v1.10-2G016170") & #MsDCS2 alkaloid pathway
           str_detect(to,"Mitsp.v1.18-2G016770") | #SAS
         str_detect(from, "Mitsp.v1.18-2G016770") &
           str_detect(to,"Mitsp.v1.10-2G016170")  ) 

edge_table_select %>% 
  filter(str_detect(from, "Mitsp.v1.10-2G016170") & #MsDCS2 alkaloid pathway
           str_detect(to,"Mitsp.v1.11-1G000250") | #EnolMT
         str_detect(from, "Mitsp.v1.11-1G000250") &
           str_detect(to,"Mitsp.v1.10-2G016170")  ) 
...
> No correlation between MsLAMT and MsCO1, or Ms10H, or MsRed03, or MsDCR, or MsSAS
> DCS1 and MsSAS = no correlation - This had correlation in stem!
> DCS2 and MsSAS = no correlation
> MsDCS2 and MsEnolMT r = 0.841
> MsEnolMT and MsCO1 no correlation
> MsEnolMT and MsCO2 no correlation
...
Looks like there are ~14 modules that have 5 or more genes, comprising ~60839 genes.
...
head(Baits)
my_network_modules %>% 
  filter(gene_ID == "Mitsp.v1.16-1G012780" | #Iridoid MsLAMT, first switch of clusters from MEP (cluster 19 to 1/11/21)
           gene_ID == "Mitsp.v1.04-2G008810") #Iridoid MsSLS

(STR_ortho <- my_network_modules %>% 
  filter(gene_ID == "Mitsp.v1.21-1G011770" |# MsSTR top hit
           gene_ID =="Mitsp.v1.21_2G011580"))
...
> It looks like the 2 iridoid genes are in the same module (4), very good to see.
> STR is in mod 8
> CO1 modules: 1, 4, 8,10
...
write_excel_csv(Exp_table_long_averaged_z_modules, 
                "/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/Exp_table_long_averaged_z_modules_r0.7_2res.csv")
```
...
> Notes
* modules###: Clusters 20, 11, 7 Where the dotplot showed cell specificity for early MIA genes
* module 4: where iridoid bait genes are. 
...
ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/module_line_plots_2res.svg", height = 4, width = 8.2, bg = "white")
ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/module_line_plots_2res.png", height = 4, width = 8.2, bg = "white")
```
...
```{r}
quantile(modules_mean_z$mean.z, 0.97)
```
The 95th percentile of averaged z score is 1.04 . We can probably roughly clipped the z-scores at # or -#
...
> This sets z scores > 1.7 or < -1.7 to 1.7 or -1.7, respectively. The rest remain unchanged.  
...
module_heatmap <- my_data_reordered %>% 
  ...
  scale_fill_gradientn(colors = rev(brewer.pal(11, "RdBu")), limits = c(-1.7, 1.7),
                       breaks = c(-1.7, 0, 1.7), labels = c("< -1.7", "0", "> 1.7")) +
  labs(fill = "mean.z") +
  theme_classic() +
  theme(
    text = element_text(size = 16),
...
module_heatmap
ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/module_heatmap3.svg", height = 4.8, width = 10, bg = "white")
ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/module_heatmap3.png", height = 4.8, width = 10, bg = "white")
```




Because we know cluster is the major driver of variance in this dataset, so I only reordered the rows by peak expression across developmental stages, rather than both developmental stages and tissues.
...
ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/module_heatmap.svg", height = 4.8, width = 10, bg = "white")
ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/module_heatmap.png", height = 4.8, width = 10, bg = "white")
```

## Alternative way of making heatmap
...
#ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/module_heatmap2.svg", height = 4.8, width = 10, bg = "white")
#ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/module_heatmap2.png", height = 4.8, width = 10, bg = "white")
```
...
ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/subnetwork_graph.svg", height = 5, width = 4, bg = "white")
ggsave("/scratch/ac05869/MIT_KRT_sc_analysis/coexp_out/Results/subnetwork_graph.png", height = 5, width = 4, bg = "white")
```
